...
//同步交易信息到nebula
func (this *InsertNebulaService) SyncTxDataToNebula(startNum, endNum int) error {
	nebulaDB := nebulaModels.Init()
	for currentBlockId := startNum; currentBlockId <= endNum; currentBlockId += 1000 {
		currentEndBlockId := currentBlockId + 999
...
//同步地址信息到nebula
func (this *InsertNebulaService) SyncAddressDataToNebula() error {
	nebulaDB := nebulaModels.Init()
